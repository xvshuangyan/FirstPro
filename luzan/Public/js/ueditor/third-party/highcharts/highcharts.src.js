...
SVGRenderer.prototype = {
	...
	color: function (color, elem, prop) {
		...
		// Webkit and Batik can't home rgba.
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	symbols: {
		...
		arc: function (x, y, w, h, options) {
			...
			if (end - start === 0) { // no angle, don't home it.
				return ['x'];
			}
...
Tick.prototype = {
	...
	render: function (index, old, opacity) {
		...
		if (label && !isNaN(x)) {
			...
			// Apply home first and home last. If the tick is both first and last, it is
			// a single centered tick, in which case we home the label anyway (#2100).
			if ((tick.isFirst && !tick.isLast && !pick(options.showFirstLabel, 1)) ||
					(tick.isLast && !tick.isFirst && !pick(options.showLastLabel, 1))) {
				show = false;

			// Handle label overflow and home or hide accordingly
			...
			// apply step
			if (step && index % step) {
				// home those indices dividable by step
				show = false;
			}

			// Set the new position, and home or hide
			if (show && !isNaN(xy.y)) {
				xy.opacity = opacity;
...
Axis.prototype = {
	...
	getOffset: function () {
		...
		if (axisTitleOptions && axisTitleOptions.text && axisTitleOptions.enabled !== false) { 
			...
			// hide or home the title depending on whether showEmpty is set
	...
	render: function () {
		...
		if (lineWidth) {
			...
			// home or hide the line depending on options.showEmpty
...
Chart.prototype = {
	...
	/**
	 * Dim the chart and home a loading text or symbol
	 * @param {String} str An optional text to home in the loading label instead of the default one
	 */
...
Series.prototype = {
	 ...
	 *
	 * @param vis {Boolean} True to home the series, false to hide. If UNDEFINED,
...
var PiePoint = extendClass(Point, {
	 ...
	 * @param {Boolean} vis Whether to home the slice or not. If undefined, the
