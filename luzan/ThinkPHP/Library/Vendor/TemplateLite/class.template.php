...
class Template_Lite {
	...
	function register_resource($type, $functions)
	{
		...
		{
			$this->_plugins['public'][$type] = $functions;
		}
		else
		{
	...
	{
		unset($this->_plugins['public'][$type]);
	}
	...
	function _get_resource($file)
	{
		...
		else
		{
			...
			$exists = isset($this->_plugins['public'][$this->_resource_type]) && call_user_func_array($this->_plugins['public'][$this->_resource_type][1], array($file, &$resource_timestamp, &$this));
...
//		$this->_error_level = error_reporting(E_ALL);
	...
	function _fetch_compile($file)
	{
		...
		{
			call_user_func_array($this->_plugins['public'][$this->_resource_type][0], array($file, &$file_contents, &$this));
		}
