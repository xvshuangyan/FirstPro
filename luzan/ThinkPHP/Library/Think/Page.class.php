...
class Page{
    ...
    /**
     * 配置静态页面（非thinkphp原生方法）
     * @return string
     */

    public function setUrl($string){
        if(isset($string) && $string != ""){
            $this->url = str_replace('$page',urlencode('[PAGE]'), $string);
        }
    }
    /**
     * 组装分页链接
    ...
    public function show() {
        ...
        if(!$this->url)$this->url = U(ACTION_NAME, $this->parameter);//原生代码去掉if(!$this->url)
        /* 计算分页信息 */
    ...
    public function show1() {
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<li><a class="num" href="/mypro/cms/Index/'.$i.'.html">' . $page . '</a></li>';
                }else{
                    break;
                }
            }else{
