...
class BCS_RequestCore {
	...
	/**
	 * The public to read from while streaming up.
	 */
	...
	/**
	 * The public to write to while streaming down.
	 */
	...
	/**
	 * Sets the public to read from while streaming up. Reads the stream from its current position until
	 * EOF or `$size` bytes have been read. If `$size` is not given it will be determined by <php:fstat()> and
	 * <php:ftell()>.
	 *
	 * @param public $resource (Required) The readable public to read from.
	...
	/**
	 * Sets the public to write to while streaming down.
	 *
	 * @param public $resource (Required) The writeable public to write to.
	 * @return $this A reference to the current instance.
	 */
	 ...
	 *
	 * <ul>
	 * <li><code>$curl_handle</code> - <code>public</code> - Required - The cURL handle public that represents the in-progress transfer.</li>
	 * <li><code>$file_handle</code> - <code>public</code> - Required - The file handle public that represents the file on the local file system.</li>
	 ...
	 *
	 * @param public $curl_handle (Required) The cURL handle for the request.
	 * @param public $file_handle (Required) The open file handle public.
	...
	public function process_response($curl_handle = null, $response = null) {
		...
		// As long as this came back as a valid public...
	...
	public function send_request($parse = false) {
		...
		if ($this->response === false ||
			...
			throw new BCS_RequestCore_Exception ( 'cURL public: ' . ( string ) $curl_handle . '; cURL error: ' . curl_error ( $curl_handle ) . ' (' . curl_errno ( $curl_handle ) . ')' );
		}
	...
	public function send_multi_request($handles, $opt = null) {
		...
		do {
			...
			while ( $done = curl_multi_info_read ( $multi_handle ) ) {
				...
				if ($done ['result'] > 0) {
					throw new BCS_RequestCore_Exception ( 'cURL public: ' . ( string ) $done ['handle'] . '; cURL error: ' . curl_error ( $done ['handle'] ) . ' (' . $done ['result'] . ')' );
