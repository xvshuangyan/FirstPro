...
class BaiduBCS {
	...
	private function check_user_acl($acl) {
		...
		foreach ( $acl ['statements'] as $key => $statement ) {
			// user public action effect must in statement
			if (! isset ( $statement ['user'] ) || ! isset ( $statement ['public'] ) || ! isset ( $statement ['action'] ) || ! isset ( $statement ['effect'] )) {
				throw new BCS_Exception ( 'Param miss: format acl error, please check your param!' );
			}
			if (! is_array ( $statement ['user'] ) || ! is_array ( $statement ['public'] )) {
				throw new BCS_Exception ( 'Param error: user or public must be array, please check your param!' );
			}
