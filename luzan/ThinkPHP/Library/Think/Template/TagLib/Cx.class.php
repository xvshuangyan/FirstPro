...
class Cx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'import'    =>  array('attr'=>'file,href,type,value,basepath','close'=>0,'alias'=>'load,css,highcharts'),
    ...
    public function _import($tag,$content,$isFile=false,$type='') {
        ...
        if($isFile) {
            ...
            // 命名空间导入模式 默认是js
            $type       =   $type?$type:(!empty($tag['type'])?strtolower($tag['type']):'highcharts');
            ...
            foreach ($array as $val){
                ...
                switch($type) {
                case 'highcharts':
                    $parseStr .= '<script type="text/javascript" src="'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.highcharts'.($version?'?'.$version:'').'"></script>';
                    break;
                case 'css':
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="__PUBLIC__/Js/Base.highcharts" />
    ...
    // import别名使用 导入js文件 <highcharts file="__PUBLIC__/Js/Base.highcharts" />
    public function _js($tag,$content) {
        return $this->_import($tag,$content,true,'highcharts');
    }
